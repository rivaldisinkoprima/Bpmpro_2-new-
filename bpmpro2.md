...
- **PORT**: `COM11` – Port komunikasi serial yang digunakan.
...
## 2. Parsing Payload Data (`parse_packet`)
...
## 3. Sistem Pembacaan Serial (`read_serial_loop`)
Ini adalah fungsi utama (loop tak terbatas) yang terus berjalan untuk memantau konektivitas:
1. **Koneksi Port**: Mencoba menghubungkan dengan `COM11`. Jika gagal (misalnya kabel dicabut), ia akan memberi peringatan dan mencoba menghubungkan ulang setiap 3 detik.
...
## 4. Eksekusi Utama (`__main__`)
Saat skrip dijalankan, ia mencetak pesan *header* ke konsol bahwa BP Monitor masuk di mode "FIX COM11" dengan fitur "Emergency stop" lalu langsung mengeksekusi `read_serial_loop()`.

---

## Kesimpulan Alur (Flowchart Singkat)
1. **Wait/Standby**: Buka `COM11`, lalu baca satu-persatu byte hingga menemukan header `0x5A`.
